...
// }) {
//   return (
//     <View style={{ position: "absolute", top: 0, right: 8, zIndex: 10 }}>
//       <Button
//         variant="ghost"
//         size="icon"
...
export function VisitCard({
  ...
  const [expanded, setExpanded] = useState(false);
  const [menuVisible, setMenuVisible] = useState(false); // 👈 for 3-dots menu
  ...
  return (
    ...
    >
      {/* 3-dots menu trigger + menu (only for owner & when onDelete is provided) */}
      {isOwner && onDelete && (
        <>
          <Pressable
            style={{
